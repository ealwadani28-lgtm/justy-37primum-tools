    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="text-2xl font-extrabold mt-1" style={{ color: "#00e5ff" }}>SAR {total.toLocaleString()}</div>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground">صفقات مغلقة</div>
          <div className="text-2xl font-extrabold mt-1" style={{ color: "#52ec6f" }}>SAR {won.toLocaleString()}</div>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground">عدد العملاء</div>
          <div className="text-2xl font-extrabold mt-1">{leads.length}</div>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="text-2xl font-extrabold mt-1" style={{ color: "#e9b3ff" }}>
            {leads.length ? Math.round((leads.filter((l) => l.stage === 4).length / leads.length) * 100) : 0}%
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="اسم العميل" className="bg-input border border-border rounded-xl p-2.5 text-sm" />
        <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="الشركة" className="bg-input border border-border rounded-xl p-2.5 text-sm" />
        <input value={value} onChange={(e) => setValue(e.target.value)} placeholder="القيمة (SAR)" type="number" className="bg-input border border-border rounded-xl p-2.5 text-sm" />
                ...
                {stageLeads.map((l) => (
                  <div key={l.id} className="rounded-xl border border-border bg-secondary/60 p-3">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-[11px] font-bold" style={{ color: s.color }}>SAR {l.value.toLocaleString()}</span>
                    </div>
                  </div>
                ))}
              </div>
