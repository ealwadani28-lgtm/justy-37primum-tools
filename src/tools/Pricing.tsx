...
function buildHtml(yearly: boolean) {
  const cards = plans
    .map(
      (p) => `  <div class="plan${p.featured ? " featured" : ""}">
    <h3>${p.name}</h3>
    <div class="price">${yearly ? p.yearly : p.monthly} ر.س / ${yearly ? "سنة" : "شهر"}</div>
    <ul>${p.features.map((f) => `<li>✓ ${f}</li>`).join("")}</ul>
    <button>${p.cta}</button>
  </div>`
    )
    .join("\n");
  return `<section dir="rtl" class="pricing">\n${cards}\n</section>`;
}

export function Pricing() {
  const [yearly, setYearly] = useState(false);
  const html = buildHtml(yearly);
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-center gap-3 mb-8">
        ...
        <button
          ...
          className={`relative w-14 h-8 rounded-full transition ${yearly ? "bg-primary" : "bg-muted"}`}
        >
          <span
            className={`absolute top-1 w-6 h-6 rounded-full bg-white transition-all ${yearly ? "right-1" : "right-7"}`}
          />
        </button>
        <span className={yearly ? "font-bold" : "text-muted-foreground"}>
          سنوي <span className="text-xs text-primary">(خصم 17%)</span>
        </span>
      </div>
        ...
        {plans.map((p) => (
          <div
            ...
            className={`rounded-3xl p-6 border ${
              p.featured ? "border-primary bg-card-glow shadow-glow" : "border-border bg-secondary"
            }`}
          >
            {p.featured && (
              <div className="text-xs bg-mint-gradient text-primary-foreground inline-block px-3 py-1 rounded-full font-bold mb-3">
                الأكثر شيوعاً
              </div>
            )}
      ...
      <ActionBar
        ...
        downloadText={{ content: html, filename: "pricing.html", mime: "text/html" }}
        downloadLabel="تحميل HTML"
        share={{ title: "جدول الأسعار" }}
      />
    </div>
  );
}
