...
import { FileText, Highlighter, MessageSquare, PenLine, Share2, ChevronsLeft, MousePointer2, Check } from "lucide-react";
...
export function PdfViewer() {
  ...
  return (
    <div className="space-y-8" dir="rtl">
      {/* Hero Showcase — light, premium, layered */}
      <div className="relative overflow-hidden rounded-3xl border border-[#e6e8ec] bg-gradient-to-br from-[#f4f6f8] via-[#eef1f5] to-[#e8ecf1] p-8 md:p-14 shadow-[0_30px_80px_-30px_rgba(15,23,42,0.25)]">
        {/* Soft ambient blobs */}
        ...
        {/* Stacked PDF cards (fan effect) */}
        <div className="relative z-10 mx-auto h-72 md:h-80 w-full max-w-2xl">
          {/* Back-left card */}
          <DocCard
            title="عقد إيجار"
            className="absolute top-8 right-1/2 translate-x-[140%] md:translate-x-[160%] -rotate-[14deg] w-44 md:w-52"
            accent="amber"
          />
          {/* Back-right card */}
          <DocCard
            title="التقرير المالي السنوي"
            className="absolute top-8 left-1/2 -translate-x-[140%] md:-translate-x-[160%] rotate-[14deg] w-44 md:w-52"
            accent="sky"
            chart
          />
          {/* Front center card */}
          <DocCard
            title="عقد إيجار"
            className="absolute top-0 right-1/2 translate-x-1/2 w-52 md:w-60 z-10"
            accent="amber"
            cursor
          />
          ...
          <div className="absolute top-16 md:top-20 left-1/2 md:left-[58%] -translate-x-1/2 z-20 w-64 rounded-2xl bg-white border border-slate-200 shadow-[0_30px_60px_-20px_rgba(15,23,42,0.35)] p-4">
            ...
            <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/60 h-20 flex items-center justify-center relative">
              <span className="text-3xl font-signature italic text-slate-700" style={{ fontFamily: "'Brush Script MT', cursive" }}>
                Signature
              </span>
      ...
      <div className="space-y-3">
        <form onSubmit={(e) => { e.preventDefault(); setUrl(input); setShowViewer(true); }} className="flex gap-2">
          <input
            value={input}
            ...
            dir="ltr"
          />
          <button className="bg-mint-gradient text-primary-foreground font-semibold px-5 rounded-xl">عرض</button>
        </form>
        {showViewer && (
...
function ToolbarBtn({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <button className="flex flex-col items-center gap-0.5 px-2.5 py-1 rounded-lg hover:bg-white/5 transition group">
